...
if False: # distance of the barrier NB is the independent variable
    ...
    for Evali in range(len(Evals)):
        ...
        for sourcei in range(n_loc_dof):
            ...
            # determine kNB*
            if(sourcei==0): kNBstar = kNBvals[np.argmin(Rvals[:,sourcei])];
             
            # plot R_\sigma vs NBvals
            for sigmai in range(n_loc_dof):
                axes[sourcei,sigmai].plot(kNBvals/np.pi, Rvals[:,sigmai], color=mycolors[Evali], marker=mymarkers[1+Evali], markevery=mymarkevery, linewidth=mylinewidth);
...
if False: # energy is the independent variable
    ...
    for kNBvali in range(len(kNBvals)):
        ...
        for sourcei in range(n_loc_dof):
            ...
            for Evali in range(len(Evals)):
                ...
                NBval = int(kNBvals[kNBvali]/k_rho);
                print(NBval); #assert False;
            ...
            # determine rhoJa*
            if(sourcei==0): rhoJstar = rhoJvals[np.argmin(Rvals[:,sourcei])];
                 
            # plot R_\sigma vs NBvals
            for sigmai in range(n_loc_dof):
                axes[sourcei,sigmai].plot(rhoJvals, Rvals[:,sigmai], color=mycolors[kNBvali], marker=mymarkers[1+kNBvali], markevery=mymarkevery, linewidth=mylinewidth);
                axes[sourcei,sigmai].set_ylabel("$R_"+str(ylabels[sigmai])+"$");
                axes[sourcei,sigmai].set_ylim(0.0,1.0);
