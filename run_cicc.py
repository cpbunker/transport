...
if get_data: # must actually compute data
    ...
    siam_current.DotData(nleads, nelecs, ndots, tf, dt, params, spinstate = spinstate, prefix = "dat/cicc/"+str(spinstate)+"/", verbose = verbose);
    ...
    title = "Ciccarello model, initial state |"+spinstate+" $\\rangle$";
    paramstr = "$V_b$ = "+str(Vb)+"\n$V_g$ = "+str(Vg)+"\n$U$ = "+str(U)
    plot.PlotObservables(datafs, sites = ['L1','L2','LD','0','RD','R1','R2'], splots = splots, mytitle = title, paramstr = paramstr);
