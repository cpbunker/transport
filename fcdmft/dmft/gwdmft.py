...
def kernel(dmft, mu, wl=None, wh=None, occupancy=None, delta=None,
    ...
    if dmft.save_mf:
        if rank == 0:
            ...
            feri['eri_imp'] = np.asarray(eri_imp)
            feri.close()
        comm.Barrier()
...
def imp_ham(hcore_cell, eri_cell, bath_e, bath_v, ncore):
    '''
...
def get_bath_direct(hyb, freqs, nw_org):
    ...
    nw = len(freqs)
    wmult = nw // nw_org
    assert(wmult > 0 and nw % nw_org != 0);
    wh = max(freqs)
    wl = min(freqs)
