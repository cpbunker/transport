...
def mf_gf(mf, freqs, delta, ao_orbs=None, verbose = 0):
    '''Calculate the mean-field GF matrix in AO basis'''
    mo_coeff = mf.mo_coeff
    mo_energy = mf.mo_energy
    if len(mo_coeff.shape) == 2: # handle RHF case by wrapping in spin up
...
def dmrg_rdm(mf, ao_orbs=None, n_threads=7, cas=False, casno='gw', composite=False,
             ...
             gs_noises=[1E-3, 1E-5, 1E-7, 1E-11, 0], local=True,
             load_dir=None, save_dir=None, load_cas=False):
    '''Calculate the DMRG GF matrix in the AO basis'''
...
def fci_kernel(mf, verbose = 0):
    ...
    Nel_up = nel / 2
    Nel_down = nel / 2
    nel, Nel_up, Nel_down = int(nel/2), int(nel/2), 0; # convert back to ASU
    Irrep = 0
    maxMemWorkMB = 100.0
    FCIverbose = 0
    ...
    if(verbose): print(">> in fci_kernel()\n - gd = ", EnergyCheMPS2,"\n - nelec = ",nel,"(already determined by mean field calc)" );
    if rank == 0:
...
def fci_gf(mf, freqs, delta, ao_orbs=None, gmres_tol=1e-4, verbose = 0):
    assert(fci_);
    if ao_orbs is None:
        nmo = mf.mo_coeff.shape[0]
        ao_orbs = range(nmo)
    ...
    fcisol = fci_kernel(mf, verbose = verbose)
    theFCI = fcisol.FCI
    ...
    # put gf in proper shape 
    return np.array([gf[:nao,:nao]]); 

def fci_rdm(mf, ao_orbs=None, verbose = 0):
    if ao_orbs is None:
        nmo = mf.mo_coeff.shape[0]
        ao_orbs = range(nmo)
    nao = len(ao_orbs)
...
def fci_sol_to_rdm(mf, fcisol, nao):
    # my addition, just avoids doing fci solution twice
    nmo = mf.mo_coeff.shape[0]
    theFCI = fcisol.FCI
    gs_vector = fcisol.GSvector
    rdm2 = np.zeros(nmo**4) 
