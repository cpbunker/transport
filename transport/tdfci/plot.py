...
def snapshot_bench(psi_ci, psi_mps, eris_inst, driver_inst, params_dict, savename,
    ...
    if(psi_mps is not None): # with dmrg
        ...
        for obsi in range(len(obs_strs)):
            ...
            x_js, y_js = vs_site(js_pass,psi_mps,driver_inst,True,obs_strs[obsi]);
            axes[obsi].plot(x_js,y_js,color=mycolors[0],marker='o',
                               label = ("DMRG ($C"+str(concur_sites)+"=${:.2f})").format(C_dmrg));
            
            # save DMRG data
            if(not plot_fig):
                if(obsi==0): # only one x_js needed
                    np.save(savename[:-4]+"_arrays/xjs_time{:.2f}".format(time), arrs[arri]);
                np.save(savename[:-4]+"_arrays/"+obs_strs[obsi]+"yjs_time{:.2f}".format(time), arrs[arri]);
