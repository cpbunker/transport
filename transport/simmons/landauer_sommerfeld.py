...
#    assert (kBT == 0.0); # just to have same call signature
    ...
    for n in ns:
        ...
        muLE = muL-Enval
        muRE = muR-Enval
        current += np.arctan((muLE)/(2*Gamma));
        current +=-np.arctan((muRE)/(2*Gamma));
        denL = muLE*muLE+4*Gamma*Gamma;
        denR = muRE*muRE+4*Gamma*Gamma;
        current += -sommerfeld*4*Gamma*(muLE/(denL*denL)-muRE/(denR*denR));
        # temperature correction using Sommerfeld expansion
...
#    assert (kBT == 0.0); # just to have same call signature
    ...
    s = 3.14159265*kBT;
    sommerfeld = s*s/(6*8*Gamma*Gamma);

    # conductance
    conductance = np.zeros_like(Vb);
    for n in ns:
        Enval = En(n,EC,Vb);
        muLE = (muL-Enval)/(2*Gamma);
        muRE = (muR-Enval)/(2*Gamma);
        denL = muLE*muLE+1;
        denR = muRE*muRE+1;
        conductance += 1/denL;
        conductance += 1/denR;
        denL2 = denL*denL;
        denR2 = denR*denR;
        conductance += -sommerfeld*(1/denL2+1/denR);
        conductance += sommerfeld*4*(muLE*muLE/(denL2*denL)+muRE*muRE/(denR2*denR));
...
def dI_of_Vb(Vb, mu0, Gamma, EC, kBT, ns, Mn = 10, xvals=1e4):
    ...
    def integrand(Eint, nints, Vbint):
        ...
        for n in nints:
            ...
            ret_n = Beta/((exp_muL+1)*(1/exp_muL+1));
...
if(__name__ == "__main__"):
    ...
    if False: # plot with various methods
        ...
        for my_Gamma in [2*my_EC/np.sqrt(48), 2*my_EC/np.sqrt(48)/100]:
            ...
            # zero temp analytic
            Is = dI_of_Vb_zero(Vbs, my_mu0, my_Gamma, my_EC, my_temp, narr);
            ...
            if False:
                # zero temp gradient
                Is = np.gradient(I_of_Vb_zero(Vbs, my_mu0, my_Gamma, my_EC, my_temp, narr));
